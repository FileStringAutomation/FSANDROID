...
class _ElementKeywords(KeywordGroup):
    ...
    def element_text_should_be(self, locator, expected):
        """Get current element text and compare to expected text
        """
        element = self._element_find(locator, True, True)
        current_text = element.get_attribute(attribute)
        if current_text != expected:
            raise AssertionError("Element '%s' text should be '%s' "
                                 "but it is '%s'." % (locator, expected, current_text)) 
